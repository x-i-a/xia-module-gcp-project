...
resource "google_iam_workload_identity_pool_provider" "github_provider" {
  ...
  attribute_condition = "assertion.sub == 'repo:${each.value["repository_owner"]}/${each.value["repository_name"]}:environment:${each.value["env_name"]}' && assertion.ref.matches('${each.value["match_branch"]} && assertion.event_name == '${each.value["match_event"]}'"
